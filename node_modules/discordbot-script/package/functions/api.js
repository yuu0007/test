...
let err = client.suppress.get(message.idd)
  ...
  if(!fields && err === undefined) return message.channel.send(`:x: Invalid Property Accessors in \`$api[${inside}]\``);
...
let json = api; 
 ...
 code = code.replaceLast(`$api[${inside}]`, o)
  ...
  let link = fields[0]
  if(!link && err === undefined) return message.channel.send(":x: No Link is provided for API")
  if(!fields && err === undefined) return message.channel.send(`:x: Invalid Property Accessors in \`$api[${inside}]\``);
