...
const blackList = async(client, message, args, name, code) => {
    ...
    if (type === "server") {
        ...
        if (option === "add") {
          ...
          servers.push(ID)
           ...
           if(!servers.find(x => x === ID)) return message.channel.send(`:x: There is no blackListed server with given ID`) 
 ...
 if (option === "add") {
  if(users.find(x => x === ID)) return message.channel.send(`:x: User is already Blacklisted`);
          ...
          if(!users.find(x => x === ID)) return message.channel.send(`:x: There is no blackListed user with given ID`) 
